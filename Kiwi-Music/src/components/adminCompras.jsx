...
const Compras = () => {
    ...
    useEffect(() => {
        ...
        const fetchCompras = async () => {
            try {
                setLoading(true);
                const response = await axios.get(`https://musica-store.vercel.app/compras?page=${page}&limit=10`);
                ...
                const allProductDetails = await Promise.all(
                    compras.flatMap(compra =>
                        compra.productos.map(async (item) => {
                            try {
                                const response = await axios.get(`https://musica-store.vercel.app/productos/${item.producto}`);
